...
date: "2023-09-28"
categories: [Advertising, Causal Inference, Instrumental Variables, R]
code-fold: false
toc-expand: true
...
the group of $\text{Pr}[A = 1 | Z = 0]$ is (expected to be) 0, since those assigned to the control group should not be able to see the ad. We can pick a *degenerate* (Beta) prior distribution[^12] with $\alpha = 0,\ \beta > 0$, e.g., $\text{Beta}(0, 1)$, which results in a Beta posterior with shape parameters $\alpha = 0 + 0 = 0$ and $\beta = 30033 + 1 = 30034$. Since $\alpha = 0$, all draws from this posterior distribution are exactly zero.
