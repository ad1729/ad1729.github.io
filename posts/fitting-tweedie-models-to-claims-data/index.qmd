...
categories: [Bayesian, Bootstrap, Insurance, R, Tweedie]
...
I've never worked in the field of insurance, but I've been wanting to dive into Tweedie models for a while, since non-negative (response) variables with lots of zeroes and positive skew are pretty common and show up in many diverse discplines such across the social sciences, insurance, biology, etc.
...
The empiricial (cumulative) distribution function (EDF or eCDF) for the bootstrap distribution of the total claim amount from the population under unit exposure is shown above. The strong multimodality shows up as bumps in the eCDF but these seem pretty minor.
...
So our best guess for the distribution of $Y_i^*$'s from the Tweedie class of models is

$$
\text{Tweedie}(\mu = 217.78..., \phi = 25966.83..., p = 1.6...)
$$

(where the ellipses indicate truncation). These estimates for $\hat\mu_{\text{int}}$ and $\hat\phi_{\text{int}}$ will be used again, so they're stored as variables.
